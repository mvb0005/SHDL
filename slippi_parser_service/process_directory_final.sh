...
# Final Slippi Directory Processor
# Uses a different approach to avoid bash variable scope issues
...
# Process files and write results to temp file
find "$INPUT_DIR" -name "*.slp" -type f | while read -r file; do
...
# Count results
processed=$(grep -c "SUCCESS" "$temp_file" 2>/dev/null || echo "0")
